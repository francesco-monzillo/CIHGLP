...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    #pragma omp target enter data map(to: matrix[0:matsize], nodelabels[0:mat.nrow], edgelabels[0:mat.ncol]) map(alloc: matrix2[0:matsize])
    ...
    //DECLARING THE MATRIX2 VARIABLE INSIDE THE TARGET DEVICE FOR LATER USE (TO BE CONTINUED)
    //Will be used for tranposing the matrix
    #pragma omp declare target
    //extern bool* matrix2 = malloc(sizeof(bool) * matsize);
    #pragma omp end declare target

    // Transpose the matrix
        #pragma omp target
        {
        #pragma omp teams distribute parallel for collapse(2) shared(rows, cols, matrix)
        //#pragma omp parallel for collapse(2) shared(rows, cols, matrix, matrix2)
        ...
        //Retrieving the labels from the device.// map the data to the device
            ...
            #pragma omp target 
            #pragma omp teams distribute parallel for simd shared(rows, cols, nodelabels, matrix2) firstprivate(categoryCount, max, maxIndex, change)
            for (int i = 0; i < cols; ++i) {
                for (int j = 0; j < rows; ++j) {
                
                    int label = nodelabels[j];

                    //printf("mat_value %d\n", matrix[(i * cols) + j]);
                    //If the value is in list 1, we count it
                    //If the value is in list is 0, we do not count it
                    categoryCount[label] = categoryCount[label] + matrix2[(i * rows) + j];
                ...
                change = edgelabels[i] != maxIndex;
                edgelabels[i] = maxIndex;
    ...
    #pragma omp target exit data map(from: nodelabels[0:rows], edgelabels[0:cols]) map(release: matrix2[0:matsize])
